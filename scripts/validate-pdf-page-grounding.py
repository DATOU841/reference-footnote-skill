#!/usr/bin/env python3
"""Validate returned PDF fallback grounding checks offline."""
...
ALLOWED = {"verified", "human_review", "failed", "not_required"}
...
def main() -> int:
    ...
    for item in data.get("results", []):
        status = item.get("pdf_fallback_status")
        if status not in ALLOWED:
            errors.append(f"invalid pdf_fallback_status for {item.get('insertion_id')}: {status}")
    ...
    out = task / "state" / "pdf-fallback-verification.json"
