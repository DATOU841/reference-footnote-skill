...
from reflib import PROTECTED_TYPES, ensure_task, print_json, read_json, result, write_json


def footnote_text(ref: dict) -> str:
    ...
    for entry in evidence["claim_evidence"]:
        if entry["claim_type"] in PROTECTED_TYPES or entry["evidence_status"] in {"no_support", "conflict", "not_needed"}:
            if entry["claim_type"] in PROTECTED_TYPES or entry["evidence_status"] in {"no_support", "conflict"}:
                no_insert.append({"claim_id": entry["claim_id"], "text": entry["text"], "reason": f"{entry['claim_type']} / {entry['evidence_status']}"})
            continue
        if entry["evidence_status"] in {"strong_support", "partial_support"} and entry.get("candidates"):
            ...
            insertions.append({
                ...
                "footnote_content": {"style": args.style, "text": footnote_text(ref)},
                "evidence_basis": {
